...
# --- En# New Plotting Test
        st.subheader("📊 Plotting Test")
        st.write("Generate a plot based on input data.")
        plot_type = st.selectbox("Select Plot Type", ["Line Plot", "Bar Plot", "Scatter Plot"], key="plot_type_select")
        data_input = st.text_area(
            "Enter data (CSV format, e.g., x,y\\n1,10\\n2,20\\n3,25):",
            value="x,y\n1,10\n2,20\n3,25",
            key="plot_data_input"
        )
        plot_title = st.text_input("Plot Title", value="Sample Plot", key="plot_title_input")
        x_label = st.text_input("X-Axis Label", value="X", key="x_label_input")
        y_label = st.text_input("Y-Axis Label", value="Y", key="y_label_input")
        ...
        if st.button("Generate Plot", key="generate_plot_btn"):
            with st.spinner("Generating plot..."):
                try:
                    ...
                    # Log success and append to chat
                    output = f"✅ Plot generated successfully: {plot_type} with title '{plot_title}'"
                    formatted_output = format_latex_content(output)
                    st.session_state.messages.append({"role": "assistant", "content": formatted_output})
                    save_chat_history()
                    if st.session_state.debug_enabled:
                        logger.debug(f"Plot generated: {plot_type}, data shape: {df.shape}")
                        logger.debug(f"Appended message: {output[:50]}...")
