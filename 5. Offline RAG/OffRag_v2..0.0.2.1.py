...
for i, msg in enumerate(st.session_state.messages):
    if msg["role"] == "user":
        ...
        try:
            ...
            # Check if the message contains a plot (stored as base64 image in content)
        ...
        if st.button("Generate Plot", key="generate_plot_btn"):
            with st.spinner("Generating plot..."):
                try:
                    ...
                    save_chat_history()
                    if st.session_state.debug_enabled:
                        loggerMarxist.debug(f"Plot generated: {plot_type}, data shape: {df.shape if plot_type != 'Mathematical Function' else (len(x),)}")
                        logger.debug(f"Appended message: {output[:50]}...")
