    ...
    if uploaded_files:
        ...
        if st.button("🚀 Add to Knowledge Base", type="primary"):
            with st.spinner("Processing files..."):
                ...
                def add_file(file, app, debug_mode):
                    messages = []
                    errors = []
                    chunk_stats = []
                    try:
                        ...
                        if file.type == "application/pdf":
                            ...
                            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                                futures = [
                                    executor.submit(app.add, text, data_type="text", debug_mode=debug_mode, file_name=file.name)
                                ...
                                for future in concurrent.futures.as_completed(futures):
                                    try:
                                        ...
                                        errors.append(f"Error adding PDF page: {str(e)}")
                            ...
                            stat = app.add(ocr_text, data_type="text", debug_mode=debug_mode, file_name=file.name)
                            chunk_stats.append(stat)
                            ...
                            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                                futures = [
                                    executor.submit(app.add, chunk, data_type="text", debug_mode=debug_mode, file_name=file.name)
                                ...
                                for future in concurrent.futures.as_completed(futures):
                                    try:
                                        ...
                                        errors.append(f"Error adding text chunk: {str(e)}")
                        ...
                        messages.append(
                            f"Processed {file.name}: {successful_chunks}/{total_chunks} chunks successful, "
                        ...
                        if debug_mode:
                            logger.info(f"File processing summary for {file.name}: {status}, {successful_chunks}/{total_chunks} chunks")
                        ...
                        if 'file_path' in locals():
                            try:
                                ...
                                errors.append(f"Error deleting temporary file: {str(e)}")
                ...
                all_errors = []
                max_workers = min(10, multiprocessing.cpu_count())
                debug_mode = st.session_state.debug_mode  # Capture debug_mode in main thread
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = [
                        executor.submit(add_file, file, st.session_state.app, debug_mode)
                    ...
                    for i, future in enumerate(concurrent.futures.as_completed(futures)):
                        try:
                            ...
                            progress_text = f"Processing {uploaded_files[i].name} ({completed_files}/{total_files})"
                            with progress_placeholder.container():
                                st.progress(min(progress, 1.0), text=progress_text)
                            if debug_mode:
                                logger.info(f"Progress updated: {progress_text}, {progress*100:.1f}%")
                        except Exception as e:
                            all_errors.append(f"Thread execution error: {str(e)}")
                            logger.error(f"Thread execution error: {e}", exc_info=True)
                            completed_files += 1
                            progress = completed_files / total_files
                            progress_text = f"Error processing {uploaded_files[i].name} ({completed_files}/{total_files})"
                            with progress_placeholder.container():
                                st.progress(min(progress, 1.0), text=progress_text)
                            if debug_mode:
                                logger.info(f"Progress updated after error: {progress_text}, {progress*100:.1f}%")

                # Ensure progress bar reaches 100%
                with progress_placeholder.container():
                    st.progress(1.0, text="File processing complete")
                if debug_mode:
                    logger.info("File processing complete, progress at 100%")
