...
class DebugApp(App):
    ...
    def test_streaming(self, prompt, debug_mode=False):
        """Test streaming using Ollama raw API (bypassing embedchain)."""
        try:
            if debug_mode:
                logger.debug(f"Testing streaming with prompt: '{prompt[:50]}...'")
            start_time = time.time()
            chunk_count = 0
            ...
            with requests.post(url, json=payload, stream=True) as r:
                for line in r.iter_lines():
                    if line:
                        ...
                        if debug_mode:
                            logger.debug(
                                f"Raw chunk {chunk_count} received at {time.time() - start_time:.2f}s: "
                                f"'{chunk[:50]}...' (len={len(chunk)}), text: '{text[:50]}...'"
                            )
                        yield text
            duration = time.time() - start_time
            if debug_mode:
                logger.debug(f"Streaming test completed in {duration:.2f}s, total chunks: {chunk_count}")
            yield "\nStreaming test completed."
        except Exception as e:
            logger.error(f"Streaming test error: {str(e)}")
            yield f"Error: {str(e)}"

    def test_ollama_raw(self, prompt, debug_mode=False):
        """Test Ollama streaming directly via HTTP."""
        try:
            if debug_mode:
                logger.debug(f"Testing Ollama raw streaming with prompt: '{prompt[:50]}...'")
            start_time = time.time()
            chunk_count = 0
            url = "http://localhost:11434/api/generate"
            payload = {
            ...
            duration = time.time() - start_time
            if debug_mode:
                logger.debug(f"Ollama raw streaming test completed in {duration:.2f}s, total chunks: {chunk_count}")
            yield "\nOllama raw streaming test completed."
        except Exception as e:
            logger.error(f"Ollama raw streaming test error: {str(e)}")
            yield f"Error: {str(e)}"
...
with st.sidebar:
    if st.session_state.is_authenticated:
        ...
        st.checkbox(
            ...
            on_change=lambda: st.session_state.update(use_streaming=st.session_state.streaming_checkbox)
        )
...
# Render chat history (exclude the latest assistant message if present)
def safe_message(content, is_user, key):
    """Safely render message, handling non-serializable content."""
    if isinstance(content, types.GeneratorType):
        if st.session_state.debug_enabled:
            logger.warning(f"Generator object detected in message content at key {key}, converting to string")
        try:
            content = "".join(content)  # Consume generator
        except Exception as e:
            content = f"Error consuming generator: {str(e)}"
            logger.error(f"Failed to consume generator in message at key {key}: {str(e)}")
    elif not isinstance(content, (str, int, float, bool)):
        if st.session_state.debug_enabled:
            logger.warning(f"Non-serializable content type {type(content)} at key {key}, converting to string")
        content = str(content)
    try:
        message(content, is_user=is_user, key=key)
    except Exception as e:
        logger.error(f"Failed to render message at key {key}: {str(e)}")
        st.write(f"[Error rendering message: {str(e)}]")

if st.session_state.messages and st.session_state.messages[-1]["role"] == "assistant":
    # Render all messages except the last assistant message in bubbles
    for i, msg in enumerate(st.session_state.messages[:-1]):
        safe_message(msg["content"], is_user=msg["role"] == "user", key=str(i))
    # Render the latest assistant message as plain text
    st.write(st.session_state.messages[-1]["content"], key="current_assistant_message")
else:
    # Render all messages in bubbles if no assistant message is last
...
def ollama_raw_stream(prompt, debug_mode=False):
    """Utility function for Ollama raw streaming."""
    try:
        if debug_mode:
...
if prompt:
    ...
    with st.spinner("🔍 Analyzing..."):
        try:
            ...
            if st.session_state.streaming_active and st.session_state.use_streaming:
                ...
                if st.session_state.use_streaming:
                    # Streaming mode: Use Ollama raw streaming
                    ...
                    try:
                        for chunk in st.write_stream(response_chunks):
                            chunk_count += 1
                            text += chunk
                            st.session_state.messages[message_index]["content"] = text
                            if st.session_state.debug_enabled:
                                logger.debug(
                        ...
                        placeholder.write(f"Rendering error: {str(e)}")
                        st.session_state.messages[message_index]["content"] = f"Rendering error: {str(e)}"
                        progress_bar.progress(1.0)
                        st.session_state.streaming_active = False
                        st.rerun()
                else:
                    ...
                    try:
                        ...
                        logger.error(f"Rendering error (non-streaming): {str(e)}")
                        st.session_state.messages.append({"role": "assistant", "content": f"Rendering error: {str(e)}"})
                        st.rerun()
        except Exception as e:
            st.error(f"Chat error: {str(e)}")
            logger.error(f"Chat error: {str(e)}")
            st.session_state.messages.append({"role": "assistant", "content": f"Chat error: {str(e)}"})
            st.session_state.streaming_active = False
            st.rerun()
...
if st.session_state.is_authenticated:
    with st.sidebar.expander("🧪 RAG Component Testing", expanded=False):
        ...
        if st.button("Test Streaming", key="test_stream_btn"):
            with st.spinner("Testing streaming..."):
                try:
                    if not hasattr(st.session_state, 'app'):
                        st.session_state.messages.append({"role": "assistant", "content": "st.session_state.app is not initialized. Please restart the app."})
                        if st.session_state.debug_enabled:
                        ...
                        st.session_state.messages.append({"role": "user", "content": f"Testing streaming with prompt: {test_prompt}"})
                        if st.session_state.debug_enabled:
                            logger.debug(f"Appended message: Testing streaming with prompt: {test_prompt[:50]}...")
                        response_chunks = st.session_state.app.test_streaming(
                            test_prompt, debug_mode=st.session_state.debug_enabled
                        )
                        if not st.session_state.use_streaming:
                            ...
                            try:
                                ...
                                st.session_state.messages.append({"role": "assistant", "content": f"Rendering error: {str(e)}"})
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                            st.session_state.messages.append({"role": "assistant", "content": "Streaming test completed."})
                            if st.session_state.debug_enabled:
                                logger.debug("Appended message: Streaming test completed")
                            st.session_state.streaming_active = False
                            st.rerun()
                        else:
                            ...
                            try:
                                ...
                                logger.error(f"Rendering error (streaming): {str(e)}")
                                st.session_state.messages[message_index]["content"] = f"Rendering error: {str(e)}"
                                st.session_state.messages.append({"role": "assistant", "content": "Streaming test failed."})
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                                    logger.debug("Appended message: Streaming test failed")
                                progress_bar.progress(1.0)
                                st.session_state.streaming_active = False
                                st.rerun()
                except Exception as e:
                    st.session_state.messages.append({"role": "assistant", "content": f"Streaming test failed: {str(e)}"})
                    if st.session_state.debug_enabled:
                        logger.debug(f"Appended message: Streaming test failed: {str(e)[:50]}...")
                    st.session_state.streaming_active = False
                    st.rerun()
            ...
            with st.spinner("Testing minimal streaming..."):
                try:
                    ...
                    if st.session_state.use_streaming:
                        ...
                        progress_bar = st.progress(0)
                        chunk_count = 0
                        total_chunks_estimated = 6  # Known number of words
                        st.session_state.streaming_session_id += 1
                        session_id = st.session_state.streaming_session_id
                        st.session_state.messages.append({"role": "assistant", "content": ""})
                        message_index = len(st.session_state.messages) - 1
                        try:
                            ...
                            st.session_state.messages[message_index]["content"] = f"Minimal streaming error: {str(e)}"
                    ...
                    st.session_state.messages.append({"role": "assistant", "content": f"Minimal streaming test failed: {str(e)}"})
                    if st.session_state.debug_enabled:
                        logger.debug(f"Appended message: Minimal streaming test failed: {str(e)[:50]}...")
                    st.rerun()

        # Embedchain streaming test button
        if st.button("Test Embedchain Streaming", key="test_embedchain_stream_btn"):
            with st.spinner("Testing embedchain streaming..."):
                try:
                    if not hasattr(st.session_state, 'app'):
                        st.session_state.messages.append({"role": "assistant", "content": "st.session_state.app is not initialized. Please restart the app."})
                        if st.session_state.debug_enabled:
                        ...
                        st.session_state.streaming_active = True
                        test_prompt = "Tell me a short story."
                        st.session_state.messages.append({"role": "user", "content": f"Testing embedchain streaming with prompt: {test_prompt}"})
                        if st.session_state.debug_enabled:
                            logger.debug(f"Appended message: Testing embedchain streaming with prompt: {test_prompt[:50]}...")
                        response_chunks = ollama_raw_stream(
                            test_prompt, debug_mode=st.session_state.debug_enabled
                        )
                        if not st.session_state.use_streaming:
                            # Non-streaming default
                            ...
                            try:
                                ...
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                            st.session_state.messages.append({"role": "assistant", "content": "Embedchain streaming test completed."})
                            if st.session_state.debug_enabled:
                                logger.debug("Appended message: Embedchain streaming test completed")
                            st.session_state.streaming_active = False
                            st.rerun()
                        else:
                            # Streaming with st.write_stream
                            ...
                            try:
                                ...
                                logger.error(f"Rendering error (streaming): {str(e)}")
                                st.session_state.messages[message_index]["content"] = f"Rendering error: {str(e)}"
                                st.session_state.messages.append({"role": "assistant", "content": "Embedchain streaming test failed."})
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                                    logger.debug("Appended message: Embedchain streaming test failed")
                                progress_bar.progress(1.0)
                                st.session_state.streaming_active = False
                                st.rerun()
                except Exception as e:
                    st.session_state.messages.append({"role": "assistant", "content": f"Embedchain streaming test failed: {str(e)}"})
                    if st.session_state.debug_enabled:
                        logger.debug(f"Appended message: Embedchain streaming test failed: {str(e)[:50]}...")
                    st.session_state.streaming_active = False
                    st.rerun()

        # Ollama raw streaming test button
        if st.button("Test Ollama Raw Streaming", key="test_ollama_stream_btn"):
            with st.spinner("Testing Ollama raw streaming..."):
                try:
                    if not hasattr(st.session_state, 'app'):
                        st.session_state.messages.append({"role": "assistant", "content": "st.session_state.app is not initialized. Please restart the app."})
                        if st.session_state.debug_enabled:
                        ...
                        st.session_state.streaming_active = True
                        test_prompt = "Tell me a short story."
                        st.session_state.messages.append({"role": "user", "content": f"Testing Ollama raw streaming with prompt: {test_prompt}"})
                        if st.session_state.debug_enabled:
                            logger.debug(f"Appended message: Testing Ollama raw streaming with prompt: {test_prompt[:50]}...")
                        response_chunks = st.session_state.app.test_ollama_raw(
                            test_prompt, debug_mode=st.session_state.debug_enabled
                        )
                        if not st.session_state.use_streaming:
                            # Non-streaming default
                            ...
                            try:
                                ...
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                            st.session_state.messages.append({"role": "assistant", "content": "Ollama raw streaming test completed."})
                            if st.session_state.debug_enabled:
                                logger.debug("Appended message: Ollama raw streaming test completed")
                            st.session_state.streaming_active = False
                            st.rerun()
                        else:
                            # Streaming with st.write_stream
                            ...
                            try:
                                ...
                                logger.error(f"Rendering error (streaming): {str(e)}")
                                st.session_state.messages[message_index]["content"] = f"Rendering error: {str(e)}"
                                st.session_state.messages.append({"role": "assistant", "content": "Ollama raw streaming test failed."})
                                if st.session_state.debug_enabled:
                                    logger.debug(f"Appended message: Rendering error: {str(e)[:50]}...")
                                    logger.debug("Appended message: Ollama raw streaming test failed")
                                progress_bar.progress(1.0)
                                st.session_state.streaming_active = False
                                st.rerun()
                except Exception as e:
                    st.session_state.messages.append({"role": "assistant", "content": f"Ollama raw streaming test failed: {str(e)}"})
                    if st.session_state.debug_enabled:
                        logger.debug(f"Appended message: Ollama raw streaming test failed: {str(e)[:50]}...")
                    st.session_state.streaming_active = False
                    st.rerun()