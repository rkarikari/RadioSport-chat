...
def safe_message(content, is_user=False, key=None):
    try:
        ...
        if is_user:
            st.markdown(f"<strong>User:</strong> {formatted_content}", unsafe_allow_html=True)
        else:
            st.markdown(f"<strong>Assistant:</strong> {formatted_content}", unsafe_allow_html=True)
        # Trigger MathJax reprocessing after rendering
        st.markdown(
            "<script>reprocessMathJax();</script>",
            unsafe_allow_html=True
        )
    except Exception as e:
...
def format_latex_content(content):
    ...
    if not isinstance(content, str):
        content = str(content)
    # Escape problematic characters that might interfere with HTML
...
for i, msg in enumerate(st.session_state.messages):
    if msg["role"] == "user":
        ...
        # Assistant messages are handled by safe_message
                ...
                if st.session_state.use_streaming:
                    ...
                    try:
                        for chunk in st.write_stream(response_chunks):
                            ...
                            placeholder.markdown(f"<strong>Assistant:</strong> {formatted_text}", unsafe_allow_html=True)
                        ...
                        placeholder.markdown(f"<strong>Assistant:</strong> {formatted_error}", unsafe_allow_html=True)
                        placeholder.markdown(
                            "<script>reprocessMathJax();</script>",
                            unsafe_allow_html=True
                        )
