    ...
    def embedder(self):
        ...
        try:
            if hasattr(self, 'db') and hasattr(self.db, 'embedder'):
                ...
                if st.session_state.is_debug_enabled:
    ...
    def test_embedding(self, text):
        try:
            ...
            if isinstance(embedding, np.float32):
                if st.session_state.is_debug_enabled:
                    logger.info("Detected single float embedding, raising TypeError")
                raise TypeError("Embedding is a single float value")
            self.add(text, data_type="text", debug_mode=st.session_state.is_debug_enabled, file_name="Test Input")
            duration = time.time() - start_time
            if st.session_state.is_debug_enabled:
                logger.info(f"Embedding test successful, dimension: {len(embedding)}, duration: {duration:.2f}s")
            return {
                "success": True,
...
def initialize_app():
    try:
        ...
        try:
            st.session_state.app = get_app(db_path, debug_mode=st.session_state.is_debug_enabled)
        except Exception as e:
            logger.warning(f"Detected DB initialization error: {str(e)}, attempting reset")
            if hasattr(st.session_state, 'app'):
                try:
                    st.session_state.app.force_reset_db(debug_mode=st.session_state.is_debug_enabled)
                    if st.session_state.is_debug_enabled:
                        logger.info("Force reset DB before clearing directory")
                except Exception as e:
                    logger.warning(f"Failed to force reset DB: {str(e)}")
            try:
                safe_rmtree(db_path, debug_mode=st.session_state.is_debug_enabled)
                os.makedirs(db_path, exist_ok=True)
                st.session_state.app = get_app(db_path, debug_mode=st.session_state.is_debug_enabled)
                if st.session_state.is_debug_enabled:
...
if "is_debug_enabled" not in st.session_state:
    st.session_state.is_debug_enabled = False
if "debug_mode" not in st.session_state:
    st.session_state.debug_mode = False  # For checkbox widget only
...
if "admin_credentials" not in st.session_state:
    ...
    # Replace hashed_password with your own using bcrypt.hashpw
...
if "show_login_panel" not in st.session_state:
    st.session_state.show_login_panel = not st.session_state.is_authenticated
...
def authenticate(username, password):
    ...
    if username in st.session_state.admin_credentials:
        logger.info(f"User {username} found in credentials")
        try:
            hashed_password = st.session_state.admin_credentials[username].encode('utf-8')
            if bcrypt.checkpw(password.encode('utf-8'), hashed_password):
                logger.info(f"Authentication successful for user: {username}")
                if st.session_state.is_debug_enabled:
                    logger.info(f"Debug mode: Confirmed successful authentication for {username}")
                return True
            else:
                logger.warning(f"Authentication failed for user: {username} (incorrect password)")
                if st.session_state.is_debug_enabled:
                    logger.warning(f"Debug mode: Password check failed for {username}")
                return False
        except Exception as e:
        ...
        if st.session_state.is_debug_enabled:
            logger.warning(f"Debug mode: User {username} not found in credentials")
        return False
...
with st.sidebar:
    if st.session_state.is_authenticated:
        ...
        if uploaded_files:
            ...
            if st.button("🚀 Add to Knowledge Base", type="primary"):
                with st.spinner("Processing files..."):
                    ...
                    chunks, extract_messages, extract_errors, temp_files = extract_chunks(uploaded_files, st.session_state.is_debug_enabled)
                    ...
                    debug_mode = st.session_state.is_debug_enabled
        ...
        st.divider()
        # Sync checkbox with is_debug_enabled
        st.checkbox(
            "🐞 Enable Debug Mode",
            value=st.session_state.is_debug_enabled,
            key="debug_mode",
            on_change=lambda: st.session_state.update(is_debug_enabled=st.session_state.debug_mode)
        )
        ...
        if st.button("🔒 Logout"):
            ...
            logger.info(
                f"User logged out, session state: is_authenticated={st.session_state.is_authenticated}, "
                f"is_debug_enabled={st.session_state.is_debug_enabled}, show_login_panel={st.session_state.show_login_panel}"
            )
            st.rerun()

    elif st.session_state.show_login_panel:
        st.subheader("🔐 Admin Login")
        st.info("Use username 'admin' (case-sensitive) and password 'admin123'. Contact support if issues persist.")
        with st.form(key="login_form"):
            username = st.text_input("Username", help="Enter 'admin' (lowercase, no spaces)")
            password = st.text_input("Password", type="password", help="Enter 'admin123'")
            ...
            if submit_button:
                if not username or not password:
                    ...
                    st.error("Username must be 'admin' (lowercase)")
                    ...
                    if authenticate(username, password):
                        ...
                        logger.info("Login successful, debug mode reset")
                        st.rerun()
                    else:
                        st.error("Invalid username or password")
                        st.session_state.show_login_panel = False
        # JavaScript to hide login panel after 5 seconds
        st.markdown(
            """
            <script>
            setTimeout(function() {
                ...
                if (login_form) {
                    ...
                    // Update session state via Streamlit's rerun
                    window.parent.Streamlit.setComponentValue({show_login_panel: false});
                }
            }, 5000);
            </script>
            """,
            ...
            if not hasattr(st.session_state, 'app'):
                ...
                if st.session_state.last_uploaded_image is not None:
                    ...
                    response = st.session_state.app.chat(
                        prompt, debug_mode=st.session_state.is_debug_enabled, image=img_path
                    )
                    os.remove(img_path)
                else:
                    response = st.session_state.app.chat(
                        prompt, debug_mode=st.session_state.is_debug_enabled
                    )
...
if st.session_state.is_debug_enabled and st.session_state.is_authenticated:
    st.divider()
