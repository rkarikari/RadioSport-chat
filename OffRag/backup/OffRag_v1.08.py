...
# Include MathJax v3.2.2 for LaTeX rendering (locally hosted)
st.markdown(
    """
    ...
    <script>
        ...
        // Function to reprocess MathJax after dynamic content updates
        function reprocessMathJax() {
            if (typeof MathJax !== 'undefined' && MathJax.typeset) {
                // Target specific elements to avoid reprocessing entire page
                MathJax.typesetPromise(['.mathjax-user', '.mathjax-assistant']).then(() => {
                    console.log("MathJax reprocessed");
                }).catch(err => console.error("MathJax reprocessing error:", err));
            }
        }
    </script>
    """,
...
# Safe message rendering for user and assistant messages
def safe_message(content, is_user=False, key=None):
    try:
        ...
        if st.session_state.debug_enabled:
            logger.debug(f"Rendering {'user' if is_user else 'assistant'} message: {formatted_content[:100]}...")
        # Wrap content in a div with a specific class for MathJax targeting
        class_name = "mathjax-user" if is_user else "mathjax-assistant"
        html_content = f'<div class="{class_name}"><strong>{"User" if is_user else "Assistant"}:</strong> {formatted_content}</div>'
        st.markdown(html_content, unsafe_allow_html=True)
        # Trigger MathJax reprocessing with a slight delay
        st.markdown(
            """
            <script>
                setTimeout(function() {
...
def format_latex_content(content):
    """Detect and preserve LaTeX expressions for MathJax rendering."""
    if not isinstance(content, str):
        content = str(content)
    if st.session_state.debug_enabled:
        logger.debug(f"Formatting LaTeX content: {content[:100]}...")
    # Pre-encode LaTeX delimiters to prevent Streamlit sanitization
    content = content.replace(r'\[', r'\\[').replace(r'\]', r'\\]')
    # Escape problematic HTML characters
    content = content.replace('<', '&lt;').replace('>', '&gt;')
    # Match LaTeX delimiters: $...$, $$...$$, \[...\], \(...\), or standalone commands
    latex_pattern = r'(\$\$.*?\$\$|\$.*?\$|\\\[.*?\\\]|\\\(.*?\\\)|\\[\w{}]+)'
    parts = []
    last_end = 0
    ...
    for match in re.finditer(latex_pattern, content, re.DOTALL):
        ...
        if latex.startswith('$$') and latex.endswith('$$'):
            parts.append(latex)  # Preserve display math
        elif latex.startswith('$') and latex.endswith('$'):
            parts.append(latex)  # Preserve inline math
        elif latex.startswith(r'\\[') and latex.endswith(r'\\]'):
            # Restore encoded delimiters for MathJax
            latex = latex.replace(r'\\[', r'\[').replace(r'\\]', r'\]')
            parts.append(latex)  # Preserve \[...\]
        elif latex.startswith(r'\\(') and latex.endswith(r'\\)'):
            parts.append(latex)  # Preserve \(...\)
        else:
            # Wrap standalone commands in $
...
for i, msg in enumerate(st.session_state.messages):
    if msg["role"] == "user":
        ...
        safe_message(msg["content"], is_user=False, key=str(i))
                    ...
                    try:
                        for chunk in st.write_stream(response_chunks):
                            ...
                            placeholder.markdown(
                                f'<div class="mathjax-assistant"><strong>Assistant:</strong> {formatted_text}</div>',
                                unsafe_allow_html=True
                            )
                            # Trigger MathJax reprocessing with delay
                            placeholder.markdown(
                                """
                                <script>
                                    setTimeout(function() {
                        ...
                        placeholder.markdown(
                            f'<div class="mathjax-assistant"><strong>Assistant:</strong> {formatted_error}</div>',
                            unsafe_allow_html=True
                        )
                        placeholder.markdown(
                            """
