...
# Include MathJax v3.2.2 for LaTeX rendering with enhanced debugging
st.markdown(
    """
    ...
    <script>
        ...
        function reprocessMathJax() {
            if (typeof MathJax !== 'undefined' && MathJax.typeset) {
                console.log("Attempting to reprocess MathJax");
                const elements = document.querySelectorAll('.mathjax-message');
                if (elements.length > 0) {
                    MathJax.typesetPromise(elements).then(() => {
                        console.log("MathJax reprocessed successfully");
                    }).catch(err => {
                        console.error("MathJax reprocessing error:", err);
                    });
                } else {
                    console.warn("No mathjax-message elements found");
                }
            } else {
                console.error("MathJax not loaded or typeset unavailable");
            }
        }
        function pollForMathJax() {
            let attempts = 0;
            const maxAttempts = 20;
            const interval = setInterval(function() {
                attempts++;
                const elements = document.querySelectorAll('.mathjax-message');
                if (typeof MathJax !== 'undefined' && elements.length > 0) {
                    console.log("MathJax and elements found, reprocessing");
                    reprocessMathJax();
                    clearInterval(interval);
                } else if (attempts >= maxAttempts) {
                    console.error("MathJax not loaded or no elements after " + maxAttempts + " attempts");
                    clearInterval(interval);
                } else {
                    console.log("Polling attempt " + attempts + ": MathJax=" + (typeof MathJax) + ", elements=" + elements.length);
                }
            }, 1000);
        }
        setTimeout(pollForMathJax, 500);
    </script>
    <style>
        .mathjax-message { white-space: pre-wrap; font-size: 16px; }
        .mathjax-fallback { white-space: pre-wrap; color: #888; }
    </style>
    """,
...
class DebugApp(App):
    ...
    def chat(self, prompt, debug_mode=False, **kwargs):
        try:
            ...
            if not hasattr(response, '__iter__') or isinstance(response, (str, bytes)):
                if debug_mode:
                    logger.warning(f"Non-iterable response: {type(response).__name__}, wrapping as generator")
            ...
            for chunk in response:
                ...
                if debug_mode:
                    ...
                    logger.debug(
                        f"Raw chunk {chunk_count}: '{chunk[:50]}...' (len={len(chunk)})"
                    )
                ...
                chunk = re.sub(r'\\{2,}\)', r'\)', chunk)
                if debug_mode:
                    logger.debug(f"Normalized chunk {chunk_count}: '{chunk[:50]}...'")
            ...
            if not self.debug_info["current_session"]["response"].strip():
                if debug_mode:
                    logger.warning("Empty response, returning unavailable message")
            ...
            if debug_mode:
                ...
                logger.debug(f"Chat response completed, total chunks: {chunk_count}")
        ...
        try:
            if hasattr(self, 'db') and hasattr(self.db, 'client'):
                ...
                try:
                    ...
                    logger.warning(f"Failed to stop client system: {str(e)}")
                self.db = None
            else:
    ...
    def test_embedding(self, text):
        try:
            ...
            duration = time.time() - start_time
            if st.session_state.debug_enabled:
                logger.debug(f"Embedding test successful, dimension: {len(embedding)}")
            return {
                "success": True,
...
def ollama_raw_stream(prompt, debug_mode=False):
    try:
        ...
        with requests.post(url, json=payload, stream=True) as r:
            for line in r.iter_lines():
                if line:
                    ...
                    if debug_mode:
                        logger.debug(
                            f"Ollama raw chunk {chunk_count}: '{text[:50]}...' (len={len(text)})"
                        )
                    # Normalize LaTeX delimiters
                    ...
                    text = re.sub(r'\\{2,}\)', r'\)', text)
                    if debug_mode:
                        logger.debug(f"Ollama normalized chunk {chunk_count}: '{text[:50]}...'")
                    response_text += text
                    yield text
        duration = time.time() - start_time
        ...
        if debug_mode:
            logger.debug(f"Ollama raw streaming completed, total chunks: {chunk_count}")
    except Exception as e:
...
def process_chunk(app, file_name, chunk_text, data_type, debug_mode, completed_chunks, total_chunks):
    try:
        ...
        if debug_mode:
            logger.debug(f"Incremented completed_chunks to {completed_chunks.value}")
...
session_defaults = {
    ...
    "admin_credentials": {
        "admin": "$2b$12$G.v2ZJlD4HasyM5Yy0XYFeEysl2SCJSUX0N8RXctoLoxcHPyScf8G"
    },
...
def safe_message(content, is_user=False, key=None):
    try:
        if isinstance(content, types.GeneratorType):
            if st.session_state.debug_enabled:
                logger.warning("Generator object detected in message content")
            content = "".join(content)
        elif not isinstance(content, str):
            if st.session_state.debug_enabled:
                logger.warning(f"Non-string content type {type(content)}")
            content = str(content)

        # Normalize LaTeX delimiters early
        raw_content = content
        ...
        if st.session_state.debug_enabled:
            ...
            logger.debug(f"Normalized content: {content[:100]}...")
            logger.debug(f"Rendering message (is_user={is_user})")
        formatted_content = format_latex_content(content)
        if st.session_state.debug_enabled:
            logger.debug(f"Formatted content: {formatted_content[:100]}...")
        role = "User" if is_user else "Assistant"
        html_content = (
            f'<div class="mathjax-message"><strong>{role}:</strong> {formatted_content}</div>'
        )
        ...
        if st.session_state.debug_enabled:
            st.markdown(
                f'<div class="mathjax-fallback"><strong>{role} (Fallback):</strong> {formatted_content}</div>',
                unsafe_allow_html=True
            )
    except Exception as e:
        logger.error(f"Error rendering message: {str(e)}")
        st.markdown(f"Error rendering message: {str(e)}", unsafe_allow_html=True)

def format_latex_content(content):
    if not isinstance(content, str):
        content = str(content)
    if st.session_state.debug_enabled:
        logger.debug(f"Formatting LaTeX content: {content[:100]}...")
    latex_pattern = r'(\$\$.*?\$\$|\$.*?\$|\\\[.*?\\\]|\\\(.*?\\\)|\\[\w{}]+)'
    parts = []
    last_end = 0
    ...
    for match in re.finditer(latex_pattern, content, re.DOTALL):
        ...
        latex = match.group(0)
        if st.session_state.debug_enabled:
            logger.debug(f"Matched LaTeX: {latex}")
        if latex.startswith('$$') and latex.endswith('$$'):
            ...
            # Fallback: wrap unrecognized LaTeX commands in $$
    ...
    result = ''.join(parts)
    if st.session_state.debug_enabled:
        logger.debug(f"Formatted LaTeX result: {result[:100]}...")
    return result
...
if prompt:
    if not isinstance(st.session_state.messages, list):
        st.session_state.messages = []
        logger.warning("st.session_state.messages was not a list, reinitialized")
    ...
    with st.spinner("🔍 Analyzing..."):
        try:
            ...
            if 'streaming_active' not in st.session_state:
                st.session_state.streaming_active = False
                logger.warning("Initialized missing st.session_state.streaming_active")
                ...
                if st.session_state.use_streaming:
                    ...
                    try:
                        for chunk in st.write_stream(response_chunks):
                            ...
                            if st.session_state.debug_enabled:
                                logger.debug(f"Streaming formatted text: {formatted_text[:100]}...")
                            placeholder.markdown(
                                f'<div class="mathjax-message"><strong>Assistant:</strong> {formatted_text}</div>',
                                unsafe_allow_html=True
                            )
                            placeholder.markdown(
                                """
                            ...
                            if st.session_state.debug_enabled:
                                logger.debug(
                                    f"Streamed chunk {chunk_count}: '{chunk[:50]}...' (len={len(chunk)})"
                                )
                        ...
                        formatted_error = format_latex_content("Sorry, the requested information is unavailable at this time")
                        placeholder.markdown(
                            f'<div class="mathjax-message"><strong>Assistant:</strong> {formatted_error}</div>',
                            unsafe_allow_html=True
                        )
                        placeholder.markdown(
                            """
