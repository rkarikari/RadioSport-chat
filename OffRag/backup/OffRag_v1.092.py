...
st.markdown(
    ...
    <script>
        ...
        // Fallback to reprocess after delay
        setTimeout(function() {
    ...
    <style>
        .mathjax-message { white-space: pre-wrap; }
    </style>
    """,
...
def safe_message(content, is_user=False, key=None):
    try:
        ...
        # Double-escape backslashes to prevent Streamlit sanitization
        content = content.replace('\\', '\\\\')
        formatted_content = format_latex_content(content)
        if st.session_state.debug_enabled:
            logger.debug(f"Rendering message (is_user={is_user}): {formatted_content[:100]}...")
            logger.debug(f"Raw content before formatting: {content[:100]}...")
        ...
        # Fallback rendering for sanitization issues
        if st.session_state.debug_enabled:
            st.markdown(
