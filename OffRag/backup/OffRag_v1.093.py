    ...
    <script>
        ...
        setTimeout(function() {
            if (typeof MathJax !== 'undefined') {
                ...
                console.error("MathJax not loaded after initial delay");
            }
        }, 2000);
    </script>
    <style>
        ...
        with requests.post(url, json=payload, stream=True) as r:
            for line in r.iter_lines():
                if line:
                    ...
                    if debug_mode:
                        logger.debug(
                            f"Ollama raw chunk {chunk_count}: '{chunk[:50]}...' (len={len(chunk)})"
                        )
                    response_text += text
                    yield text
        duration = time.time() - start_time
...
def safe_message(content, is_user=False, key=None):
    try:
        ...
        # HTML escape to prevent Streamlit sanitization, preserve LaTeX backslashes
...
def format_latex_content(content):
    ...
    # Unescape HTML entities for LaTeX backslashes
                ...
                if st.session_state.use_streaming:
                    ...
                    try:
                        for chunk in st.write_stream(response_chunks):
                            chunk_count += 1
                            text += chunk
                            formatted_text = format_latex_content(escape(text))
                        ...
                        formatted_error = format_latex_content(escape("Sorry, the requested information is unavailable at this time"))
                        placeholder.markdown(
